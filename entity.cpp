...
void update_entities()
{
    ...
    /** Last full-scan scoreboard: resolves names on fast ticks when EntityPtr cache misses. */
    static std::unordered_map<uint32_t, std::string> scoreboardNamesByNetId;
...

    {
        ...
        if (!tmp_entities.empty()) {
            ...
            if (fullScan) {
                ...
                if (scoreboard_size > 0 && scoreboard_size <= 120 && Game::IsValidPtr2((void*)scoreboard)) {
                    ...
                    // Build ID → name map (Arma string object: length @+8, chars @+0x10 — not a raw C string)
                    std::unordered_map<uint32_t, std::string> idToName;
                    for (uint32_t i = 0; i < scoreboard_size; i++) {
                        if (identityIDs[i] < 1 || !identityNamePtrs[i] || !Game::IsValidPtr2((void*)identityNamePtrs[i]))
                            continue;
                        std::string nm = ReadArmaString(identityNamePtrs[i]);
                        if (!nm.empty())
                            idToName[identityIDs[i]] = std::move(nm);
                    }
                    scoreboardNamesByNetId = idToName;
                ...
                for (size_t i = 0; i < tmp_entities.size(); ++i) {
                    ...
                    if (it != cachedPlayerInfo.end()) {
                        ...
                        tmp_entities[i].playerName.clear();
                        tmp_entities[i].itemInHands.clear();
                        auto nit = scoreboardNamesByNetId.find((uint32_t)tmp_entities[i].NetworkID);
                        if (nit != scoreboardNamesByNetId.end())
                            tmp_entities[i].playerName = nit->second;
                    }
                }
            }
        }
        ...
        // Camera as late as possible vs. entity coords/bones so WorldToScreen matches current frame data
        RefreshCamera();
        markPass("RefreshCamEnd");
...
void keyboard_listener()
{
    while (true)
    {
        const int menuVk = GameVars.menuToggleHotkey ? GameVars.menuToggleHotkey : VK_INSERT;
        if (!g_menuToggleUsesRegisterHotkey && (GetAsyncKeyState(menuVk) & 1))
        {
            GameVars.menuShow = not GameVars.menuShow;
        }
        const int dmaVk = GameVars.dmaRefreshHotkey ? GameVars.dmaRefreshHotkey : VK_F3;
        if (GetAsyncKeyState(dmaVk) & 1)
        {
            ...
            {
                char logb[96];
                snprintf(logb, sizeof(logb), "[%s] Manual full DMA refresh triggered...", GetKeyName(dmaVk));
                Log(logb);
                VMMDLL_ConfigSet(vmm, VMMDLL_OPT_REFRESH_ALL, 1);
                snprintf(logb, sizeof(logb), "[%s] Refresh complete.", GetKeyName(dmaVk));
                Log(logb);
            }
        }
