...
inline namespace DayZ
{
	...
	public:
		...
		/** System-wide menu toggle (RegisterHotKey + polling fallback). Default Insert. */
		int menuToggleHotkey = VK_INSERT;
		/** Full DMA cache refresh (keyboard_listener). Default F3. */
		int dmaRefreshHotkey = VK_F3;
		...
		/** First launch after start: opaque black fill until menu is opened once. */
		bool blackStartupOverlay = true;
		/** Hide Windows taskbar while cheat runs (restored on exit). */
